...
public class UserPdfController {
    ...
    public ResponseEntity<byte[]> getPivotalReport(@AuthenticationPrincipal AuthorizedUser user,
        ...
        ReportFile file = reportCreator.create(workInfoService.getWorkInfos(from, to, Optional.of(user.getId()), Optional.empty(), Optional.empty(), Optional.empty()), INDIVIDUAL_PIVOTAL, user.getFullName());
